...
public class BeanViajes extends Trip implements Serializable {
	...
	public String verParticipantes(Trip trip) throws NotPersistedException{
		...
		List<Seat> seats = Factories.services.createSeatsService().findByTrip(trip.getId());
		List<User> users = new ArrayList<User>();
		for (Seat s : seats){
			User u = Factories.services.createUsersService().findById(s.getUserId());
			users.add(u);
		}
	...
	public void controlarEstadoViajes() throws NotPersistedException{
		TripService ts = Factories.services.createTripsService();
	...
	public String disponibles() throws NotPersistedException {
		...
		SeatService ss =  Factories.services.createSeatsService();
		ApplicationService as =  Factories.services.createApplicationsService();
		List<Trip> trips = Factories.services.createTripsService().getTrips();
	...
	public String misSolicitudes() {
		...
		List<Application> applications = Factories.services.createApplicationsService().getApplications();
		List<Seat> seats = Factories.services.createSeatsService().getSeats();
		TripService ts = Factories.services.createTripsService();
		for(Seat s : seats){
	...
	public String verPromotor(Trip trip){
		...
		User user = null;
		try {
			user = Factories.services.createUsersService().findById(trip.getPromoterId());
	...
	public SeatStatus getSeatStatus(Long tripId , Long userId){
		Seat s = Factories.services.createSeatsService().findByUserAndTrip(userId, tripId);
		return s.getStatus();
	}
