...
public class BeanApplications {
	...
	public String cargarSolicitudes(Trip trip){
		...
		setTrip(trip);
		setApplications(Factories.services.createApplicationsService().findByTrip(getTrip().getId()));
		setSeats(Factories.services.createSeatsService().findByTrip(getTrip().getId()));
	...
	public String getNombreBySeat(Seat seat){
		User user = null;
		try {
			user = Factories.services.createUsersService().findById(seat.getUserId());
	...
	public String getNombreByApplication(Application app){
		User user = null;
		try {
			user = Factories.services.createUsersService().findById(app.getUserId());
		} catch (NotPersistedException e) {
			Log.error("No se ha encontrado al usuario");
	...
	public boolean solicitado(Trip t){
		Application ap = Factories.services.createApplicationsService()
	...
	public String solicitarPlaza(Trip trip) {
		...
		if (!trip.getPromoterId().equals(getUserId())) {
			...
			try {
				Factories.services.createApplicationsService().saveApplication(
						application);
	...
	public String confirmarPlaza(Application application) {
		String resultado = "exito";
		setTrip(Factories.services.createTripsService().findById(
		...
		if (getTrip().getAvailablePax() > 0) {
			if (getTrip().getClosingDate().after(new Date())) {
				...
				try {
					Factories.services.createTripsService().updateTrip(getTrip());
					Factories.services.createApplicationsService()
				...
				try {
					Factories.services.createSeatsService().saveSeat(seat);
				} catch (AlreadyPersistedException e) {
					resultado = "error";
					Log.error("No se ha podido confirmar la solicitud");
	...
	public String cambiarEstadoSeat(Seat seat) throws NotPersistedException{
		...
		if(seat.getStatus().equals(SeatStatus.ACCEPTED)){
			...
			Factories.services.createSeatsService().updateSeat(seat);
			getTrip().setAvailablePax(getTrip().getAvailablePax()+1);
			Factories.services.createTripsService().updateTrip(getTrip());
