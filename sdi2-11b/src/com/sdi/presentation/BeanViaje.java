...
public class BeanViaje extends Trip implements Serializable {
	...
	public void tripInicialize() {
		...
		setClosingDate(calendar.getTime());
		calendar.add(Calendar.DAY_OF_MONTH, 2);
		setDepartureDate(calendar.getTime());
		calendar.add(Calendar.DAY_OF_MONTH, 3);
		setArrivalDate(calendar.getTime());
		setPlazasDisponibles("4");
	...
	public String saveTrip() throws AlreadyPersistedException{
		...
		if(Factories.services.createTripsService().findByPromoterAndArrivalDate(this.getPromoterId(), this.getArrivalDate()) != null){
			resultado = false;
		...
		if(resultado){
			...
			FacesContext.getCurrentInstance().addMessage (null, new FacesMessage(FacesMessage.SEVERITY_INFO, "INFO" ,"Viaje creado con éxito"));
			return "exito";
		}
		return "";
	}
	...
	public String updateTrip() throws NotPersistedException{
		...
		Trip t = Factories.services.createTripsService().findByPromoterAndArrivalDate(this.getPromoterId(), this.getArrivalDate());
	...
	public String cancelTrip(Trip trip){
		...
		SeatService seatService = Factories.services.createSeatsService();
		List<Seat> seats = seatService.findByTrip(trip.getId());
		ApplicationService applicationService= Factories.services.createApplicationsService();
